...
public class Principal {
    ...
    private void buscar5TopSeries() {
        List<Serie> topSeries = repositorio.findTop5ByOrderByEvaluacionDesc();

        topSeries.forEach(e -> System.out.println(e.getTitulo()));
    }

    private void buscarSeriesPorTitulo() {
        System.out.print("Name of the series you want to search for:");
        var nombre = stdin.nextLine();

        Optional<Serie> serieBuscada = repositorio.findByTituloContainsIgnoreCase(nombre);

        serieBuscada.ifPresent(e -> {
            System.out.println("Serie encontrada: " + e);
        });
    }

    private void mostrarSeriesBuscadas() {
        series = /*datosSeries.stream().map(
                Serie::new
        ).toList();
        repositorio.findAll();
    ...
    private void buscarEpisodioPorSerie() throws JsonProcessingException {
        ...
        System.out.println("Select the series form which you want to know their episodes");
        var nombre = stdin.nextLine();
        ...
        serie.ifPresent(serieEncontrada -> {

            List<DatosTemporada> temporadas = new ArrayList<>();

            System.out.println("Serie: " + serieEncontrada.getTitulo());

            for (int i = 1; i <= serieEncontrada.getTotalTemporadas(); i++) {
                var json = consumer.obtenerDatos(URL_BASE + "&t=" + URLEncoder.encode(serieEncontrada.getTitulo(), StandardCharsets.UTF_8) + "&season=" + i);
...
public class Principal {
    ...
    private final String URL_BASE = "https://www.omdbapi.com/?apikey=d5bee84b&type=series&t=";
    private ConvierteDatos conversor = new ConvierteDatos();
    private List<DatosSerie> datosSeries = new ArrayList<>();
    private SerieRepository repositorio;
    private List<Serie> series;
